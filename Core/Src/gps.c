	...
	
		{
			...
			if(value_len>=6)
			{
				...
				g_gpstime.Seconds=(p[4]-'0')*10+(p[5]-'0');
					...
					p=p+value_len+1;
					value_len=GetAsciiValueLen(p,','); //???????speed  7
					...
					p=p+value_len+1;
					value_len=GetAsciiValueLen(p,','); //?????????8

					//g_MotionDirect=atoi(p);


					p=p+value_len+1;	
					value_len=GetAsciiValueLen(p,','); //???? ddmmyy  9
			...
			if(p!=NULL)
			{
				...
				{
					p=p+6;
					value_len=GetAsciiValueLen(p,',');//ddmm.mmmm  ???
					...
					p=p+value_len+1;
					value_len=GetAsciiValueLen(p,','); //dddmm.mmmm  ????
					...
					p=p+value_len+1;
					value_len=GetAsciiValueLen(p+6,',');//???????? A=VALILD  V=????
