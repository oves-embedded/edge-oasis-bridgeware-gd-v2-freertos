...
*/
const TASK_LIST_TypeDef g_Task[]=
{
	{TIMER_UPLOAD,COUNT_SUB,&g_GprsUpload_timer,TimerGprsUploadCallback,"GprsUpload",configMINIMAL_STACK_SIZE,TIMER_COUNT+1},
	{TIMER_CAMP,COUNT_SUB,&g_Camp_timer,TimerCampCallback,"Camp",configMINIMAL_STACK_SIZE,TIMER_COUNT},
	{TIMER_PUMP,COUNT_SUB,&g_Pump_timer,TimerPumpCallback,"Pump",configMINIMAL_STACK_SIZE,TIMER_COUNT-1},
	{TIMER_PAYG,COUNT_SUB,&g_Payg_timer,TimerPaygCallback,"Payg",configMINIMAL_STACK_SIZE,TIMER_COUNT-2},
	{TIMER_BMS_COM,COUNT_SUB,&g_BmsCom_timer,TimerBmsComCallback,"BmsCom",configMINIMAL_STACK_SIZE,TIMER_COUNT-3},
	{TIMER_AT_ACKOUT,COUNT_SUB,&g_AtAckTout_timer,NULL,"AtAckTout",configMINIMAL_STACK_SIZE,TIMER_COUNT-4},
	{TIMER_MQTT,COUNT_SUB,&g_Mqtt_timer,NULL,"Mqtt",configMINIMAL_STACK_SIZE,TIMER_COUNT-5},
	{TIMER_GSMCOM,COUNT_SUB,&g_GsmCom_timer,NULL,"GsmCom",configMINIMAL_STACK_SIZE,TIMER_COUNT-6},
	{TIMER_KEY,COUNT_ADD,&g_KeyTimer,TimerKeyScan,"Key",configMINIMAL_STACK_SIZE,TIMER_COUNT-20},
	{TIMER_SLEEP,COUNT_SUB,&g_SleepTimer,NULL,"Sleep",configMINIMAL_STACK_SIZE,TIMER_COUNT-8},
	{TIMER_LCD_BL,COUNT_SUB,&g_Backlight_timer,TimeBacklightCallback,"Backlight",configMINIMAL_STACK_SIZE,TIMER_COUNT-9},
	{TIMER_LCDSAVER,COUNT_SUB,&g_LcdSaver_timer,TimeSaverCallback,"LcdSaver",configMINIMAL_STACK_SIZE,TIMER_COUNT-10},
	{TIMER_KB_INSERT,COUNT_SUB,&g_KbPwrHoldTimer,TimeKbPwrHoldCallback,"KbPwrHold",configMINIMAL_STACK_SIZE,TIMER_COUNT-11},
	{TIMER_UI_FLASH,COUNT_SUB,&g_UiFlashTimer, TimeUiFlashCallback,"UiFlash",configMINIMAL_STACK_SIZE,TIMER_COUNT-12},
	{TIMER_CL_FULL,COUNT_SUB,&g_ClFullTimer, TimeCoulomCallback,"ClFull",configMINIMAL_STACK_SIZE,TIMER_COUNT-13},
	{TIMER_CL_CHRGE_OVER,COUNT_SUB,&g_ClChrgeOverTimer, NULL,"ClChrgeOver",configMINIMAL_STACK_SIZE,TIMER_COUNT-14},
	{TIMER_CL_CHRGE_LOW,COUNT_SUB,&g_ClChrgeLowTimer, NULL,"ClChrgeLow",configMINIMAL_STACK_SIZE,TIMER_COUNT-15},
	{TIMER_SYS_INIT,COUNT_SUB,&g_SysInitTimer, NULL,"SysInit",configMINIMAL_STACK_SIZE,TIMER_COUNT-16},
	{TIMER_DEMO,COUNT_SUB,&g_DemoTimer, NULL,"Demo",configMINIMAL_STACK_SIZE,TIMER_COUNT-17},
	{TIMER_GPS_SAMPLE,COUNT_SUB,&g_GpsSampleTimer, NULL,"GpsSample",configMINIMAL_STACK_SIZE,TIMER_COUNT-18},
	{TIMER_RAML_REPROT,COUNT_SUB,&g_RamLReprotTimer, TimeRamlReportCallback,"RamlReport",configMINIMAL_STACK_SIZE,TIMER_COUNT-19},
	{TIMER_ABAC_SAMPLE,COUNT_SUB,&g_AbacSampleTimer, TimeAbacSampleCallback,"AbacSample",configMINIMAL_STACK_SIZE,TIMER_COUNT-20},
};
...
void oldtaskCreate(void)
{
	uint8_t i;

	for(i=0;i<sizeof(g_Task)/sizeof(TASK_LIST_TypeDef);i++)
	{
		if(g_Task[i].p->enable)
			xTaskCreate(g_task_wrapper, g_Task[i].pcName, g_Task[i].usStackDepth, &i, g_Task[i].uxPriority, NULL);
	}
}
void g_task_wrapper(void *pvParameters)
{
	...
	uint8_t i = *((uint8_t *)pvParameters);
	g_Task[i].p->event=TRUE;
	if(g_Task[i].p->period==FALSE)
		g_Task[i].p->enable=FALSE;
	if(g_Task[i].callback!=NULL)
		g_Task[i].callback(g_Task[i].p);
	vTaskDelay(g_Task[i].p->count);
}
